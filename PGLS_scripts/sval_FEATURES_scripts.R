...
#This script is tring to measure what could be associated with translational selection (Y=s_val, X= Genomic features)
...
repeats_masked <- read.delim("all_1154_repeatmasker_output.tbl")
...
#Extract bases masked (how much of the genome was masked of repeats)
masked_repeats <- repeats_masked[which(repeats_masked$type == "bases_masked"),]
...
#when doing it normally just do it with 5
index <- 4

while(index <= 15) {
  
  #Running agaonst Features Files
  #when doing it normally just do it with 4
  column <- 4
  ...
  while(column <= 4) {
    ...
    if (column == 6 || column == 10 || column == 11){
      ...
      summ <- pgls.possible( yeast_Features_mergeDF[,c(index)] ~ masked_repeats[,c("percentage")], comparative.data(all_tree,yeast_Data_mergeDF,"Tip_ID_on_tree"), lambda = "ML" )
      codvs <- append(codvs, print(paste0(names(yeast_Features_mergeDF)[index]," Vs ", names(yeast_Features_mergeDF)[column])))
    ...
    summ <- pgls.possible( yeast_features[,c(index)] ~ masked_repeats[,c("percentage")], comparative.data(all_tree,yeast_data,"Tip_ID_on_tree"), lambda = "ML" )
    codvs <- append(codvs, print(paste0(names(yeast_features)[index]," Vs ", names(yeast_features)[column])))
